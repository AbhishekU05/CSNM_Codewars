...
def logic(arena_data: dict):
    ...
    "Prince": ["Skeleton", "Minion", "Barbarian", "Wizard"], 
    ...
    for troop_name in opp_troop_data.keys():
        ...
        if weakness[troop_name][0] == "everything":
            if opp_troop_data[troop_name][1] <= 20 and not troop_nearby:
        ...
        # React if weakness is wizard or prince as they are op
        ...
        if my_troop_data:
            ...
            y_choice = 0    
            if elixr < 9.7 and not my_troop_data and not troop_deployed:
                wait = True
        for mirror in weakness[troop_name]:
            ...
            if opp_troop_data[troop_name][1] <= 50:
                ...
                if wait: # We wait for the opponent troops to come to us if they are on their side and we don't have enough elixr
    ...
    # Do not go on offensive if we do not have elixr and there are no troops on board
    if elixr < 9.7 and not opp_troop_data and not my_troop_data:
        offense = False
